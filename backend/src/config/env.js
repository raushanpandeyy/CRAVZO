...
const envSchema = z.object({
  ...
  DATABASE_CONNECTION_LIMIT: z.coerce.number().int().positive().default(8),
  DATABASE_POOL_TIMEOUT_SECONDS: z.coerce.number().int().positive().default(5),
  DATABASE_TRANSACTION_MAX_WAIT_MS: z.coerce.number().int().positive().default(3000),
  DATABASE_TRANSACTION_TIMEOUT_MS: z.coerce.number().int().positive().default(8000),
  REDIS_CONNECT_TIMEOUT_MS: z.coerce.number().int().positive().default(1500),
  REDIS_MAX_RECONNECT_RETRIES: z.coerce.number().int().nonnegative().default(5),
  REDIS_COMMAND_TIMEOUT_MS: z.coerce.number().int().positive().default(1000),
  REQUEST_LOG_SAMPLE_RATE: z.coerce.number().min(0).max(1).default(0),
  SLOW_REQUEST_THRESHOLD_MS: z.coerce.number().int().positive().default(500),
  MEMORY_CACHE_MAX_ENTRIES: z.coerce.number().int().positive().default(500),
  METRICS_TOKEN: z.string().optional(),
  SERVER_SHUTDOWN_GRACE_MS: z.coerce.number().int().positive().default(25000),
  HTTP_KEEP_ALIVE_TIMEOUT_MS: z.coerce.number().int().positive().default(65000),
  HTTP_HEADERS_TIMEOUT_MS: z.coerce.number().int().positive().default(66000),
