...
const getRestaurantById = async (restaurantId) => {
  const response = await apiRequest(API_ENDPOINTS.restaurant.byId(restaurantId));
  return response.data;
};

const listMenuItems = async (restaurantId) => {
  const response = await apiRequest(`/api/v1/menu-items/restaurant/${restaurantId}`);
...
const searchRestaurantsAndDishes = async (query, { lat, lng, radius = 8 } = {}) => {
  ...
  const response = await apiRequest(`${API_ENDPOINTS.restaurant.list}/search?${params.toString()}`);
