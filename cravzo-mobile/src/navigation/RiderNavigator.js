  ...
  return (
    <View style={{ paddingBottom: 16, backgroundColor: "transparent" }}>
      <View style={{ marginHorizontal: 16 }}>
        <View className="flex-row rounded-3xl border border-amber-100 bg-white/95 p-1.5 shadow-xl shadow-amber-900/10">
          {state.routes.map((route, index) => {
            ...
            return (
              <TouchableOpacity key={route.key} onPress={onPress} className="flex-1 items-center gap-0.5 rounded-2xl px-1 py-1.5">
                <View className={`h-8 w-8 items-center justify-center rounded-xl ${isFocused ? "bg-amber-500 shadow-md shadow-amber-500/20" : ""}`}>
                  <Icon size={16} color={isFocused ? "#fff" : colors.slate[500]} />
                </View>
                <Text className={`text-[10px] font-extrabold ${isFocused ? "text-amber-700" : "text-slate-500"}`}>{route.name}</Text>
              </TouchableOpacity>
            );
          })}
        </View>
